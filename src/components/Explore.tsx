    ...
    <section className="bg-white">
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 py-16 sm:py-24">
        <div data-spotlight className="spotlight rounded-[20px] bg-white border border-slate-100 p-6 sm:p-10 lg:p-16 grid lg:grid-cols-[1fr_1.1fr] gap-10 lg:gap-14 items-center">
          <div>
            <div className="eyebrow-rule text-[13px] font-semibold tracking-[0.28em] mb-3" style={{ color: 'var(--accent-deep)' }}>{t.explore.insightsEyebrow}</div>
            <div className="font-jp text-[18.5px] tracking-[0.18em] text-slate-500 mb-6">{t.explore.insightsSub}</div>
            <h2 className="font-serif fluid-40-48 leading-[1.15] font-medium text-slate-900 tracking-[-0.015em] mb-6">
              {t.explore.insightsTitle}
            </h2>
            <p className="text-[23px] leading-[1.65] text-slate-600 mb-3 max-w-md">
          ...
          <div data-anim="research-cards" className="flex flex-col gap-4 min-w-0">
            {t.explore.research.map((c, i) => (
              ...
              >
                <div className="w-14 h-[70px] sm:w-16 sm:h-20 rounded-md flex-shrink-0 flex items-center justify-center text-white font-serif font-medium text-[24px] sm:text-[29px]"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="font-serif text-[20.5px] sm:text-[23px] font-semibold text-slate-900 leading-tight mb-1 truncate">{c.title}</div>
                  <div className="text-[13px] text-slate-500 truncate">{c.meta} · <span className="font-jp">{c.sub}</span></div>
                </div>
      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 pb-16 sm:pb-24">
        <div className="eyebrow-rule text-[13px] font-semibold tracking-[0.28em] mb-3" style={{ color: 'var(--accent-deep)' }}>{t.explore.apartEyebrow}</div>
        <div className="font-jp text-[18.5px] tracking-[0.18em] text-slate-500 mb-6">{t.explore.apartSub}</div>
          ...
          {t.explore.features.map((c, i) => (
            <div key={i} data-spotlight className="spotlight group bg-white border border-slate-100 rounded-xl overflow-hidden hover:shadow-[0_16px_50px_-16px_rgba(var(--shadow-rgb),0.16)] hover:-translate-y-0.5 transition-all">
              ...
              <div className="p-6 sm:p-8">
                <div className="text-[12px] font-semibold tracking-[0.22em] text-slate-400 mb-1">{c.tag}</div>
                <div className="font-jp text-[14.5px] tracking-[0.16em] text-slate-500 mb-3">{c.sub}</div>
                <h3 className="font-serif text-[26.5px] font-semibold text-slate-900 mb-3 tracking-[-0.01em]">{c.title}</h3>
                <p className="text-[21px] leading-[1.6] text-slate-600 mb-7">{c.body}</p>
                <a href="#" className="link-underline text-[18.5px] font-semibold inline-flex items-center gap-1.5 hover:gap-2 transition-all" style={{ color: 'var(--accent-deep)' }}>
                  {c.cta}
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M13 6l6 6-6 6" /></svg>
                </a>
