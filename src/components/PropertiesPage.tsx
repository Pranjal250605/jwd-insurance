...
function AllocationDonut() {
  ...
  return (
    <div className="border border-slate-200 rounded-2xl bg-white p-5 sm:p-7 h-full">
      <div className="text-[11.5px] font-semibold tracking-[0.24em] uppercase mb-1" style={{ color: 'var(--accent-deep)' }}>{t.propsPage.alloc.kicker}</div>
      <h3 className="font-serif text-[24px] font-semibold text-slate-900 mb-6 tracking-[-0.01em]">{t.propsPage.alloc.title}</h3>
      <div className="flex items-center gap-8 flex-wrap">
        <div className="relative flex-shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
            {focus ? (
              <>
                <span className="text-[19.5px] font-bold text-slate-900 leading-none">{(focus.frac * 100).toFixed(1)}%</span>
                <span className="text-[11.5px] text-slate-500 mt-1 max-w-[90px] leading-tight">{focus.short}</span>
              </>
            ) : (
              <>
                <span className="text-[16px] font-bold text-slate-900 leading-none">AED {fmtAed(total / 1e6)}M</span>
                <span className="text-[11px] text-slate-500 mt-1">{t.propsPage.alloc.total}</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-[220px] flex flex-col gap-2.5">
          {slices.map((s) => (
            ...
            >
              ...
              <span className="flex-1 text-[16px] text-slate-700">{s.short}</span>
              <span className="text-[14.5px] text-slate-500">AED {fmtAed(s.value)}</span>
              <span className="text-[15.5px] font-semibold text-slate-900 w-12 text-right">{(s.frac * 100).toFixed(1)}%</span>
            </div>
          ))}
        </div>
      </div>
      <p className="text-[13.5px] text-slate-400 mt-5">{t.propsPage.alloc.note}</p>
    </div>
  );
}
...
function AreaBars() {
  ...
  return (
    <div className="border border-slate-200 rounded-2xl bg-white p-5 sm:p-7 h-full">
      <div className="text-[11.5px] font-semibold tracking-[0.24em] uppercase mb-1" style={{ color: 'var(--accent-deep)' }}>{t.propsPage.areas.kicker}</div>
      <h3 className="font-serif text-[24px] font-semibold text-slate-900 mb-6 tracking-[-0.01em]">{t.propsPage.areas.title}</h3>
      <div className="flex flex-col gap-4">
        {AREA_PRICES.map((a) => (
          <div key={a.en} className="grid grid-cols-[64px_1fr_54px] sm:grid-cols-[110px_1fr_74px] items-center gap-2 sm:gap-3">
            <span className="text-[13px] sm:text-[15.5px] text-slate-600 truncate">{ja ? a.ja : a.en}</span>
            <div className="h-5 rounded-r-[4px] relative" style={{ width: `${(a.value / max) * 100}%`, background: 'var(--accent-deep)', opacity: a.highlight ? 1 : 0.45 }}>
              {a.highlight && (
                <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 text-[11px] font-bold tracking-wider whitespace-nowrap px-2 py-0.5 rounded-full" style={{ background: 'var(--accent-soft)', color: 'var(--accent-deep)' }}>
                  {t.propsPage.areas.highlight}
                </span>
              )}
            </div>
            <span className="text-[15.5px] font-semibold text-slate-900 text-right">{a.value.toLocaleString()}</span>
          </div>
        ))}
      </div>
      <p className="text-[13.5px] text-slate-400 mt-6">{t.propsPage.areas.note}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-baseline justify-between">
        <span className="text-[15.5px] text-slate-600">{label}</span>
        <span className="text-[16px] font-semibold text-slate-900">{display}</span>
      </div>
      <input
  ...
  return (
    <div className="border border-slate-200 rounded-2xl bg-white p-5 sm:p-8 lg:p-10">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          <div className="text-[11.5px] font-semibold tracking-[0.24em] uppercase mb-1" style={{ color: 'var(--accent-deep)' }}>{s.kicker}</div>
          <h3 className="font-serif text-[28px] font-semibold text-slate-900 tracking-[-0.01em]">{s.title}</h3>
        </div>
        <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 px-4 py-2 text-[14.5px] text-slate-500">
      ...
      <div className="flex items-center gap-2 flex-wrap mb-8">
        {LISTINGS.map((l, i) => (
          <button
            key={l.id}
            onClick={() => setIdx(i)}
            className="px-4 h-11 rounded-full border text-[14.5px] font-semibold transition-all"
            style={idx === i
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_300px]">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2">
            <span className="inline-flex items-center gap-2 text-[14.5px] text-slate-700">
        ...
        <div className="flex flex-col gap-6 rounded-2xl border border-slate-200 bg-slate-50 p-6">
          <span className="text-[11.5px] font-semibold tracking-[0.2em] uppercase text-slate-500">{s.assumptions}</span>
      ...
      <div className="mt-8 grid sm:grid-cols-3 gap-px rounded-2xl overflow-hidden border border-slate-200 bg-slate-200">
        <div className="bg-white p-5">
          <div className="text-[12.5px] uppercase tracking-[0.14em] text-slate-500 mb-1">{s.resThis} · {years}{s.yrsSuffix}</div>
          <div className="font-serif text-[28px] font-medium" style={{ color: 'var(--accent-deep)' }}>{oku(dFinal)}</div>
          <div className="text-[14px] text-slate-500 mt-0.5">AED {fmtAed(dFinal / FX_JPY)}</div>
        </div>
        <div className="bg-white p-5">
          <div className="text-[12.5px] uppercase tracking-[0.14em] text-slate-500 mb-1">{s.resJapan} · {years}{s.yrsSuffix}</div>
          <div className="font-serif text-[28px] font-medium" style={{ color: JAPAN_LINE }}>{oku(jFinal)}</div>
          <div className="text-[14px] text-slate-500 mt-0.5">{s.afterTax}</div>
        </div>
        <div className="p-5" style={{ background: 'var(--accent-soft)' }}>
          <div className="text-[12.5px] uppercase tracking-[0.14em] text-slate-500 mb-1">{s.resDiff}</div>
          <div className="font-serif text-[28px] font-medium" style={{ color: gap >= 0 ? 'var(--accent-deep)' : JAPAN_LINE }}>
            {gap >= 0 ? '+' : ''}{oku(gap)}
          </div>
      ...
      <p className="mt-6 text-[14.5px] leading-[1.7] text-slate-400">{s.disclaimer}</p>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-3 mb-2">
        <span className="text-[11.5px] font-semibold tracking-[0.18em] text-slate-500 uppercase">{t.propsPage.chartTitle}</span>
      </div>
      <div
        ref={wrapRef}
      ...
      >
        ...
        {h != null && hover != null && (
          ...
          >
            ...
            <div className="text-[11px] text-white/70 whitespace-nowrap">≈ AED {fmtAed(h.value * listing.sizeSqft)}</div>
          </div>
        )}
      </div>
      <div className="flex justify-between text-[11.5px] text-slate-400 mt-1">
        {series.filter((_, i) => i % 2 === 0).map((s, i) => <span key={`${s.label}-${i}`}>{s.label}</span>)}
      </div>
      <p className="text-[13.5px] text-slate-400 italic mt-2">{t.propsPage.chartNote}</p>
    </div>
  );
}
    ...
    <main className="bg-white">
      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 pt-10 sm:pt-16 pb-10 sm:pb-12">
        <div className="mb-8 sm:mb-10">
          <a href="#" className="relative inline-flex items-center gap-2 min-h-[44px] text-[16.5px] font-medium text-slate-500 hover:text-slate-900">
        ...
        <div className="eyebrow-rule text-[11.5px] font-semibold tracking-[0.28em] mb-3" style={{ color: 'var(--accent-deep)' }}>{t.propsPage.eyebrow}</div>
        <div className="font-jp text-[16.5px] tracking-[0.18em] text-slate-500 mb-6">{t.propsPage.sub}</div>
        ...
        <p className="text-[19.5px] leading-[1.65] text-slate-600 max-w-2xl mb-12">{t.propsPage.intro}</p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px rounded-2xl overflow-hidden border border-slate-200 bg-slate-200">
          {[
            ...
            <div key={l} className="bg-white p-6">
              <div className="font-serif text-[20.5px] sm:text-[32.5px] font-medium tracking-tight leading-tight" style={{ color: 'var(--accent-deep)' }}>{v}</div>
              <div className="text-[14px] text-slate-500 mt-1">{l}</div>
            </div>
          ))}
        </div>
      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 pb-16 flex flex-col gap-8 sm:gap-10">
              ...
              <div>
                <div className="img-zoom relative h-[260px] overflow-hidden">
                  ...
                  <div className="absolute top-4 left-5">
                    <span className="text-[11px] font-bold tracking-[0.2em] px-3 py-1.5 rounded-full bg-white/90 backdrop-blur" style={{ color: 'var(--accent-deep)' }}>{t.propsPage.acquired}</span>
                  </div>
                  <div className="absolute bottom-4 left-5 text-white">
                    <div className="text-[11.5px] font-semibold tracking-[0.2em] opacity-85 uppercase">{p.area}</div>
                    <div className="font-serif text-[26px] font-semibold leading-tight">{ja ? p.nameJa : p.nameEn}</div>
                  </div>
                </div>
                <div className="p-5 sm:p-7">
                  <div className="flex items-center gap-4 flex-wrap mb-4 text-[15.5px] text-slate-500">
                  ...
                  <div className="flex items-end justify-between gap-4 mb-5">
                    <div>
                      <div className="text-[11.5px] font-semibold tracking-[0.18em] text-slate-400 uppercase mb-1">{ja ? '取得価格' : 'Acquisition price'}</div>
                      <div className="font-serif text-[30px] font-semibold tracking-tight text-slate-900">AED {fmtAed(p.priceAed)}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[11.5px] font-semibold tracking-[0.18em] text-slate-400 uppercase mb-1">{t.propsPage.metrics.grossYield}</div>
                      <div className="font-serif text-[30px] font-semibold tracking-tight" style={{ color: 'var(--accent-deep)' }}>{p.yieldPct.toFixed(1)}%</div>
                    </div>
                  </div>
                  <p className="text-[17px] leading-[1.65] text-slate-600 mb-6">{ja ? p.descJa : p.descEn}</p>
                  <div className="flex items-center gap-3 flex-wrap">
                    {[{ label: 'Bayut', url: p.bayut }, { label: 'Property Finder', url: p.pf }].map((l) => (
                      <a key={l.label} href={l.url} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-4 h-11 rounded-full border text-[15.5px] font-bold transition-all hover:-translate-y-0.5"
              ...
              <div className="p-5 sm:p-7 lg:border-l border-t lg:border-t-0 border-slate-100 flex flex-col gap-7">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-px rounded-xl overflow-hidden border border-slate-200 bg-slate-200">
                  {[
                    ...
                    <div key={l} className="bg-white p-4">
                      <div className="font-serif text-[20.5px] font-semibold tracking-tight text-slate-900">{v}</div>
                      <div className="text-[12.5px] text-slate-500 mt-0.5 leading-tight">{l}</div>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  <div className="flex items-baseline justify-between mb-3">
                    <span className="text-[11.5px] font-semibold tracking-[0.18em] uppercase" style={{ color: 'var(--accent-deep)' }}>{t.propsPage.valueHistory}</span>
                    <span className="text-[14px] text-slate-500">{t.propsPage.since} <b style={{ color: 'var(--accent-deep)' }}>+{totalGrowth.toFixed(0)}%</b></span>
                  </div>
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-px rounded-xl overflow-hidden border border-slate-200 bg-slate-200">
                    {valueHistory.map((v) => (
                      <div key={v.year} className="p-3" style={{ background: v.now ? 'var(--accent-soft)' : 'white' }}>
                        <div className="text-[11.5px] text-slate-500">{v.year}{v.now ? ` ${t.propsPage.now}` : ''}</div>
                        <div className="text-[13px] font-semibold text-slate-900 mt-0.5 break-words">AED {fmtAed(v.value)}</div>
                        {v.yoy != null
                          ? <div className="text-[11.5px] mt-0.5" style={{ color: 'var(--accent-deep)' }}>▲ {Math.abs(v.yoy).toFixed(1)}%</div>
                          : <div className="text-[11.5px] text-slate-400 mt-0.5">{t.propsPage.base}</div>}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* risks & hedges */}
        <div className="border border-slate-200 rounded-2xl bg-white p-5 sm:p-8 lg:p-10">
          <div className="text-[11.5px] font-semibold tracking-[0.24em] uppercase mb-6" style={{ color: 'var(--accent-deep)' }}>{t.propsPage.risksTitle}</div>
          <div className="grid md:grid-cols-2 gap-4">
            {t.propsPage.risks.map((r, i) => (
              <div key={i} className="rounded-xl border border-slate-100 p-5">
                <div className="flex items-start gap-2.5">
                  <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-rose-400" />
                  <span className="text-[16px] font-semibold text-slate-900">{r.risk}</span>
                </div>
                <div className="mt-2 flex items-start gap-2.5 pl-4">
                  ...
                  <span className="text-[16px] leading-[1.6] text-slate-600">{r.hedge}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <p className="text-[14.5px] leading-[1.7] text-slate-400 max-w-4xl">{t.propsPage.disclaimer}</p>
      </div>
    </main>
  );
}
