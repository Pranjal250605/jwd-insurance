    ...
    <>
      ...
      {!open && (
        ...
        >
          ...
          <span className="text-[15.5px] font-semibold tracking-wide">{a.label}</span>
        </button>
      )}
        ...
        >
          ...
          <div className="pt-safe flex items-center justify-between border-b border-slate-100 px-4 py-3 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[15.5px] font-semibold text-slate-900 leading-tight">{a.title}</div>
                <div className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                  <span className="text-[11px] uppercase tracking-wide text-slate-400">{a.online}</span>
                </div>
              </div>
            </div>
          ...
          <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-4 flex flex-col gap-3">
            <div className="text-[15.5px] leading-[1.6] text-slate-700 bg-slate-50 rounded-2xl rounded-tl-md px-4 py-3 max-w-[92%]">
              {a.greeting}
            </div>
            {msgs.length === 0 && (
              <div className="flex flex-col items-start gap-2 mt-1">
                {a.suggestions.map((s) => (
                  <button key={s} onClick={() => send(s)}
                    className="px-3.5 min-h-[44px] rounded-full border text-[14px] font-medium transition-all hover:-translate-y-0.5 text-left"
            ...
            {msgs.map((m, i) => (
              m.role === 'user' ? (
                <div key={i} className="self-end text-[15.5px] leading-[1.6] text-white rounded-2xl rounded-br-md px-4 py-2.5 max-w-[85%]" style={{ background: 'var(--accent-deep)' }}>
                  {m.content}
                </div>
              ) : (
                <div key={i} className={`text-[15.5px] leading-[1.6] rounded-2xl rounded-tl-md px-4 py-3 max-w-[92%] ${m.error ? 'bg-rose-50 text-rose-700' : 'bg-slate-50 text-slate-700'}`}>
                  {m.content ? (
          ...
          <div className="flex-shrink-0">
            ...
            >
              <input
                ...
                className="flex-1 h-11 px-4 rounded-full border border-slate-200 text-[17px] outline-none focus:border-slate-400 bg-white"
              />
              <button
                type="submit"
            ...
            <div className="pb-safe px-4 pb-2 text-[11px] text-slate-400 text-center">{a.disclaimer}</div>
          </div>
        </div>,
        document.body,
      )}
