      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 py-14 sm:py-20">
        <div className="grid lg:grid-cols-[1.4fr_1fr_1fr_1fr_1fr] gap-12 mb-12">
          <div>
            ...
            <p className="text-[19px] text-slate-600 leading-[1.7] max-w-xs mb-3">
              {t.footer.blurb}
            </p>
            <p className="font-jp text-[18px] text-slate-500 leading-[1.85] tracking-wide max-w-xs mb-7">
              {t.footer.blurb2}
            </p>
            <div className="flex flex-col gap-2 mb-7">
              {t.products.platforms.map((p) => (
                <a
                  ...
                  className="link-underline inline-flex items-center gap-1.5 text-[19px] font-medium text-slate-600 hover:text-slate-900 self-start"
                >
                  {p.name}
          ...
          {t.footer.cols.map((col) => (
            <div key={col.h}>
              <h4 className="text-[13px] font-bold tracking-[0.22em] text-slate-900 uppercase">{col.h}</h4>
              <div className="font-jp text-[12px] tracking-[0.16em] text-slate-400 mt-1 mb-5">{col.sub}</div>
              <ul className="space-y-3">
                {col.links.map((l) => (
                  <li key={l}>
                    <a href="#" className="link-underline text-[19px] text-slate-600 hover:text-slate-900">{l}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="pt-10 border-t border-slate-200 flex flex-col gap-6">
          <div>
            <h4 className="text-[13px] font-bold tracking-[0.22em] text-slate-900 uppercase">{t.footer.officesLabel}</h4>
            <div className="font-jp text-[12px] tracking-[0.16em] text-slate-400 mt-1 mb-6">{t.footer.officesSub}</div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-7">
              {t.footer.offices.map((o) => (
                <div key={o.name}>
                  <div className="text-[15px] font-medium text-slate-700 mb-1.5">{o.name}</div>
                  <address className="not-italic text-[14px] leading-[1.75] tracking-[0.04em] text-slate-500">
                    {o.lines.map((line) => (
          ...
          <div className="flex flex-col lg:flex-row gap-6 lg:items-start lg:justify-between">
            <p className="text-[17px] text-slate-500 leading-[1.7] max-w-3xl">
              <strong className="text-slate-700">{t.footer.importantLabel}</strong> {t.footer.disclaimer}
            </p>
            <div className="flex items-center flex-wrap gap-x-6 gap-y-2 text-[14.5px] text-slate-500">
