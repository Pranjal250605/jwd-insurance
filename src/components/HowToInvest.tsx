  ...
  return (
    <main className="bg-white">
      <section className="max-w-[1280px] mx-auto px-5 sm:px-8 pt-12 sm:pt-16 pb-10">
        <div className="eyebrow-rule text-[14.5px] font-semibold tracking-[0.22em] mb-6" style={{ color: 'var(--accent-deep)' }}>
          {t.howTo.eyebrow}
        </div>
        <h1 className="font-serif text-[41px] sm:text-[53px] font-medium tracking-[-0.015em] text-slate-900 leading-[1.2] mb-2">
          {t.howTo.title}
        </h1>
        <p className="text-[15.5px] font-semibold tracking-[0.14em] text-slate-400 mb-6">{t.howTo.sub}</p>
        <p className="text-[20.5px] leading-[1.8] text-slate-600 max-w-2xl">{t.howTo.intro}</p>
        ...
        <ol className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4 list-none p-0">
          {t.howTo.steps.map((s, i) => (
            <li key={s.title} className="border border-slate-200 rounded-2xl p-6">
              <div className="text-[12px] font-bold tracking-[0.2em] mb-3" style={{ color: 'var(--accent-deep)' }}>
                {t.howTo.stepLabel} {String(i + 1).padStart(2, '0')}
              </div>
              <h2 className="font-serif text-[24px] font-semibold text-slate-900 mb-2.5 leading-snug">{s.title}</h2>
              <p className="text-[17px] leading-[1.75] text-slate-600">{s.body}</p>
            </li>
          ))}
        </ol>

        <h2 className="mt-16 text-[25px] sm:text-[27.5px] font-bold mb-6" style={{ color: 'var(--accent-deep)' }}>
          ...
          {t.products.platforms.map((p) => (
            ...
            >
              <span className="min-w-0">
                <span className="block text-[12px] font-bold tracking-[0.18em] mb-1 opacity-70" style={{ color: 'var(--accent-deep)' }}>{p.tag}</span>
                <span className="block font-serif text-[26.5px] font-semibold tracking-tight leading-none" style={{ color: 'var(--accent-deep)' }}>{p.name}</span>
              </span>
        ...
        <p className="mt-4 text-[15px] leading-relaxed text-slate-500 max-w-2xl">{t.howTo.note}</p>
      </section>
