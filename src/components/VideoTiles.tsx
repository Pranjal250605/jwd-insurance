  ...
  return createPortal(
    ...
    >
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-[1100px]">
        ...
        />
        <div className="mt-4 text-center text-[17px] text-white/80">{label}</div>
      </div>
    </div>,
    document.body,
  );
      ...
      {t.hero.videos.map((v) => (
        <figure key={v.id} className="m-0 grid grid-cols-[minmax(0,120px)_1fr] sm:grid-cols-[minmax(0,200px)_1fr] gap-4 sm:gap-7 items-start">
          ...
          <figcaption className="min-w-0">
            <div className="text-[23px] sm:text-[26.5px] font-bold text-slate-900 leading-snug">
              {v.title}
              <span className="whitespace-nowrap">{v.note}</span>
            </div>
            {v.desc && (
              <p className="mt-2.5 text-[21.5px] sm:text-[26.5px] leading-[1.65] text-slate-800">{v.desc}</p>
            )}
          </figcaption>
        </figure>
      ))}
  ...
  return (
    <section className="bg-white">
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 pt-4 pb-12 sm:pb-16">
        <div className="text-[15px] font-bold tracking-[0.22em] text-slate-400 mb-5">{t.hero.moreVideosLabel}</div>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {t.hero.moreVideos.map((v) => (
            <figure key={v.id} className="m-0">
              ...
              <figcaption className="mt-2.5 text-[18px] font-medium text-slate-700 leading-snug">{v.title}</figcaption>
            </figure>
          ))}
        </div>
      </div>
