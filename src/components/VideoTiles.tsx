      ...
      {t.hero.videos.map((v) => (
        <figure key={v.id} className="m-0 grid grid-cols-[minmax(0,120px)_1fr] sm:grid-cols-[minmax(0,200px)_1fr] gap-4 sm:gap-7 items-start">
          ...
          <figcaption className="min-w-0">
            <div className="text-[19px] sm:text-[22px] font-bold text-slate-900 leading-snug">
              {v.title}
              <span className="whitespace-nowrap">{v.note}</span>
            </div>
            {v.desc && (
              <p className="mt-2.5 text-[18px] sm:text-[22px] leading-[1.65] text-slate-800">{v.desc}</p>
            )}
          </figcaption>
        </figure>
      ))}
