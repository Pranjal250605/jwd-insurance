    ...
    <main className="bg-white">
      <div className="max-w-[980px] mx-auto px-5 sm:px-8 pt-10 sm:pt-16 pb-28 sm:pb-16">
        <button
          ...
          className="mb-6 inline-flex items-center gap-2 text-[15.5px] font-medium text-slate-500 hover:text-slate-800"
        >
        ...
        <div className="rounded-3xl p-4 sm:p-8" style={{ background: 'var(--accent-soft)' }}>
          ...
          >
            <h1 className="text-[18px] sm:text-[20.5px] font-bold text-center leading-relaxed text-slate-900 mb-8">
              {t.consent.title}
            </h1>

            <h2 className="text-[17px] sm:text-[18px] font-bold text-center text-slate-900 mb-6">{t.consent.lead}</h2>

            <div className="space-y-3">
              {t.consent.intro.map((p, i) => (
                <p key={i} className="text-[16px] sm:text-[17px] leading-[1.9] text-slate-700">{p}</p>
              ))}
            </div>

            {t.consent.sections.map((sec) => (
              <section key={sec.heading} className="mt-9">
                <h3 className="text-[16px] sm:text-[17.5px] font-bold text-slate-900 mb-4">{sec.heading}</h3>
                <div className="space-y-3">
                  {sec.body.map((p, i) => (
                    <p key={i} className="text-[16px] sm:text-[17px] leading-[1.9] text-slate-700">{p}</p>
                  ))}
                ...
                {sec.bullets.length > 0 && (
                  <ul className="mt-4 space-y-2.5 list-none p-0">
                    {sec.bullets.map((b, i) => (
                      <li key={i} className="flex gap-2.5 text-[16px] sm:text-[17px] leading-[1.9] text-slate-700">
            ...
            <blockquote className="mt-8 border-l-2 pl-4 sm:pl-5 text-[16px] sm:text-[17px] leading-[1.9] text-slate-800" style={{ borderColor: 'var(--accent-deep)' }}>
              {t.consent.quote}
            </blockquote>
            ...
            >
              ...
              />
              <span className="text-[20.5px] sm:text-[23.5px] font-bold leading-[1.7] text-slate-900">{t.consent.agreeLabel}</span>
            </label>

            <div className="mt-8 space-y-2">
              {t.consent.notes.map((n, i) => (
                <p key={i} className="text-[15px] leading-[1.85] text-slate-500">{n}</p>
              ))}
            </div>
            <p className="mt-6 text-[16px] sm:text-[17px] leading-[1.9] font-semibold text-slate-800">{t.consent.closing}</p>
          </div>
          ...
          <div className="mt-6 flex flex-col items-center gap-3">
            <button
              ...
              className={`h-[52px] px-14 rounded-full text-[18px] font-semibold transition-colors duration-300 ${
                armed
            ...
            {armed && lang === 'ja' && (
              <p className="max-w-[46ch] text-center text-[14.5px] leading-[1.8] text-slate-500">{JA_PROXY_NOTICE}</p>
            )}
            {!reachedEnd && <p className="text-[15px] text-slate-400">{t.consent.scrollHint}</p>}
            {showHint && <p className="text-[15px] text-rose-500">{t.consent.hint}</p>}
          </div>
        </div>
      </div>
    </main>
