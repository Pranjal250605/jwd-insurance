      ...
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 pb-12">
        ...
        >
          <div className="relative">
            ...
            <div className="font-jp text-[16.5px] tracking-[0.18em] text-slate-500 mb-7">{c.sub}</div>
            ...
            <figure className="lg:float-left lg:w-[340px] lg:mr-10 lg:mb-8 mb-8 m-0">
              <div className="img-zoom relative rounded-xl overflow-hidden aspect-[4/5]">
                ...
                <figcaption className="absolute bottom-0 left-0 right-0 p-5">
                  <div className="text-white/70 text-[11.5px] tracking-[0.12em] mb-1.5">{c.company}</div>
                  <div className="text-white font-serif text-[20.5px] font-semibold leading-tight">{c.name}</div>
                  <div className="text-white/80 text-[13.5px] mt-0.5">{c.role}</div>
                </figcaption>
              </div>
            </figure>

            <p className="text-[19.5px] leading-[1.75] font-medium text-slate-800 mb-6">{c.lead}</p>

            {c.blocks.map((block, i) => (
              <div key={i}>
                {block.heading && (
                  <h3 className="text-[18px] font-semibold text-slate-900 mt-8 mb-4">{block.heading}</h3>
                )}
                {block.paragraphs.map((p, j) => (
                  <p key={j} className="text-[18px] leading-[1.9] text-slate-600 mb-4 last:mb-0">
                    {withEmphasis(p)}
                  </p>
                ))}
              </div>
